...
class plot_curve(object):
    ...
    def _update(self):
        ...
        if False:  # is_complex_dtype(np.array(dset)):
            ...
            for axis, ufunc, comp_name in zip(
                axes.flat, [np.abs, np.angle], ["Magnitude", "Phase"]
            ):
                axis.plot(
                    self.dset.dims[ref_dims][0], ufunc(np.squeeze(curve)), **kwargs
                )
                if comp_name == "Magnitude":
                    axis.set_title(
                        self.dset.file.filename.split("/")[-1]
                        + "\n("
                        + comp_name
                        + ")",
                        pad=15,
                    ...
                    axis.set_xlabel(
                        self.dset.get_dimension_labels()[ref_dims[0]]
                    )  # + x_suffix)
                    ...
                    axis.set_xlabel(
                        self.get_dimension_labels()[ref_dims[0]]
                    )  # + x_suffix)
                    axis.ticklabel_format(style="sci", scilimits=(-2, 3))
    ...
    def __init__(self, dset, dim_dict, figure=None, **kwargs):
        ...
        if is_complex_dtype(self.dset):
            ...
            for axis, ufunc, comp_name in zip(
                ...
                plot_map(
                    axis,
                    ufunc(np.squeeze(img)),
                    show_xy_ticks=True,
                    show_cbar=True,
                    cbar_label=cbar_label,
                    x_vec=ref_dims[1].values,
                    y_vec=ref_dims[0].values,
                    **kwargs
                )
                axis.set_title(self.name + "\n(" + comp_name + ")", pad=15)
                axis.set_xlabel(
                    ref_dims[1].name + " (" + ref_dims[1].units + ")" + suffix[1]
                )
                axis.set_ylabel(
                    ref_dims[0].name + " (" + ref_dims[0].units + ")" + suffix[0]
                )
            fig.tight_layout()
            return fig, axes
            ...
            self.img = self.axis.imshow(
                np.squeeze(self.dset).T, extent=extent, **kwargs
            )
            self.axis.set_title(self.dset.file.filename.split("/")[-1], pad=15)
            self.axis.set_xlabel(
                self.dset.get_dimension_labels()[dim_dict["spatial"][0]]
            )  # + x_suffix)
            self.axis.set_ylabel(
                self.dset.get_dimension_labels()[dim_dict["spatial"][1]]
            )
            self.axis.ticklabel_format(style="sci", scilimits=(-2, 3))
...
class plot_stack(object):
    ...
    def __init__(self, dset, dim_dict, figure=None, **kwargs):
        ...
        if len(image_dims) < 2:
            raise KeyError(
                "spatial key in dimension_dictionary must be list of length 2"
            )
            return
        ...
        self.axis = self.fig.add_axes([0.0, 0.2, 0.9, 0.7])
        self.ind = 0
        ...
        self.slider = Slider(
            axidx, "image", 0, self.cube.shape[0] - 1, valinit=self.ind, valfmt="%d"
        )
        ...
        self.anim = animation.FuncAnimation(
            self.fig, self._updatefig, interval=200, blit=False, repeat=True
        )
        self._update()
...
class plot_spectrum_image(object):
    ...
    def __init__(self, dset, dim_dict, figure=None, horizontal=True, **kwargs):
        ...
        if len(spec_dim) < 1:
            raise KeyError(
                "spectral key in dimension_dictionary must be list of length 1"
            )
            return
        ...
        self.fig.tight_layout()
        self.cid = self.axes[1].figure.canvas.mpl_connect(
    ...
    def get_spectrum(self):
        ...
        self.spectrum = np.average(
            self.cube[self.x : self.x + self.bin_x, self.y : self.y + self.bin_y, :],
            axis=(0, 1),
        )
        ...
        if event.inaxes in [self.axes[0]]:
            ...
            if x >= 0 and y >= 0:
                if x <= self.rectangle[1] and y <= self.rectangle[3]:
                    ...
                    self.rect.set_xy(
                        [
                            self.x * self.rect.get_width() / self.bin_x
                            + self.rectangle[0],
                            self.y * self.rect.get_height() / self.bin_y
                            + self.rectangle[2],
                        ]
                    )
