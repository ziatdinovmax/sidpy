...
def find_results_groups(h5_main, tool_name, h5_parent_group=None):
    ...
    if h5_parent_group is not None:
        if not isinstance(h5_parent_group, (h5py.File, h5py.Group)):
            raise TypeError(
                "'h5_parent_group' should either be a h5py.File " "or h5py.Group object"
            )
    else:
    ...
    for key in h5_parent_group.keys():
        if (
            dset_name in key
            and tool_name in key
            and isinstance(h5_parent_group[key], h5py.Group)
        ):
...
def check_and_link_ancillary(h5_dset, anc_names, h5_main=None, anc_refs=None):
    ...
    if isinstance(anc_names, (str, unicode)):
        anc_names = [anc_names]
    if isinstance(anc_refs, (h5py.Dataset, h5py.Group, h5py.File, h5py.Reference)):
        anc_refs = [anc_refs]
    ...
    def __check_and_link_single(h5_obj_ref, target_ref_name):
        if isinstance(h5_obj_ref, h5py.Reference):
            ...
            warnstring = "{} is not a valid h5py Reference and will be skipped.".format(
                repr(h5_obj_ref)
            )
            warn(warnstring)
...
def copy_attributes(source, dest, skip_refs=True, verbose=False):
    ...
    try:
        ...
        if not skip_refs:
            warn(
                "Dataset references will not be copied since {} and {} are "
...
def validate_main_dset(h5_main, must_be_h5):
    ...
    if len(h5_main.shape) != len(h5_main.dims):
        raise ValueError(
            "Main data does not have full set of dimensional "
...
def validate_anc_h5_dsets(h5_inds, h5_vals, main_shape, is_spectroscopic=True):
    ...
    if h5_inds.shape != h5_vals.shape:
        raise ValueError(
            "h5_inds: {} and h5_vals: {} should be of the same "
    ...
    if isinstance(main_shape, (list, tuple)):
        ...
        raise TypeError(
            "main_shape should be of the following types:"
    ...
    if h5_inds.shape[is_spectroscopic] != main_shape[is_spectroscopic]:
        raise ValueError(
            "index {} in shape of h5_inds: {} and main_data: {} "
            "should be equal".format(int(is_spectroscopic), h5_inds.shape, main_shape)
        )
...
def validate_dims_against_main(main_shape, dims, is_spectroscopic=True):
    ...
    if not isinstance(main_shape, (list, tuple)):
        raise TypeError(
            "main_shape should be a list or tuple. Provided object"
    ...
    if isinstance(dims, Dimension):
        ...
        raise TypeError(
            '"dims" must be a list or tuple of nsid.Dimension '
    ...
    if lhs != rhs:
        raise ValueError(
            dim_category + " dimensions in main data of size: {} do not match "
...
def check_if_main(h5_main, verbose=False):
    ...
    * source
    ...
    for i, dimension in enumerate(h5_main.dims):
        ...
        dset_success.append(
            np.all([attr_success, isinstance(h5_dim_dset, h5py.Dataset)])
        )
    ...
    for attr_name in main_attrs_names:
        ...
        if not isinstance(val, (str, unicode)):
            if verbose:
                print(
                    "Attribute {} of {} found to be {}. Expected a string".format(
                        attr_name, h5_main.name, val
                    )
                )
            return False
    ...
    for group in groups:
        ...
        if target_dset is not None:
            if target_dset in group.keys():
                ...
                if verbose:
                    print(
                        "{} did not contain the target dataset: {}".format(
                            group.name.split("/")[-1], target_dset
                        )
                    )
                continue
...
def get_source_dataset(h5_group):
    ...
    if len(name_split) != 2:
        raise ValueError(
            "The provided group's name could not be split by '-' as expected in "
...
def assign_group_index(h5_parent_group, base_name, verbose=False):
    ...
    for item_name in temp:
        if isinstance(h5_parent_group[item_name], h5py.Group) and item_name.startswith(
            base_name
        ):
    ...
    if verbose:
        print(
            "indices of existing groups with the same prefix: {}".format(
                previous_indices
            )
        )
...
def copy_main_attributes(h5_main, h5_new):
    ...
    for att_name in ["quantity", "units"]:
        if att_name not in h5_main.attrs:
            raise KeyError(
                "Attribute: {} does not exist in {}".format(att_name, h5_main)
            )
...
def create_empty_dataset(
    source_dset, dtype, dset_name, h5_group=None, new_attrs=None, skip_refs=False
):
    """
    ...
    if h5_group is None:
        ...
        if source_dset.file != h5_group.file and not skip_refs:
            ...
            warn(
                "H5 object references will not be copied over since {} is in "
    ...
    if source_dset.file.driver == "mpio":
        if kwargs.pop("compression", None) is not None:
            warn(
                'This HDF5 file has been opened with the "mpio" communicator. '
    ...
    if dset_name in h5_group.keys():
        if isinstance(h5_group[dset_name], h5py.Dataset):
            warn(
                "A dataset named: {} already exists in group: {}".format(
            ...
            if any(
                (source_dset.shape != h5_new_dset.shape, dtype != h5_new_dset.dtype)
            ):
                warn(
                    "Either the shape (existing: {} desired: {}) or dtype (existing: {} desired: {}) of the dataset "
                    "did not match with expectations. Deleting and creating a new one.".format(
                        h5_new_dset.shape, source_dset.shape, h5_new_dset.dtype, dtype
                    )
                )
...
def check_for_matching_attrs(h5_obj, new_parms=None, verbose=False):
    ...
    if not isinstance(h5_obj, (h5py.Dataset, h5py.Group, h5py.File)):
        raise TypeError(
            "h5_obj should be a h5py.Dataset, h5py.Group, or h5py.File object"
        )
    if new_parms is None:
        new_parms = dict()
    else:
    ...
    for key in new_parms.keys():
        ...
        if isinstance(old_value, np.ndarray):
            if not isinstance(new_parms[key], collections.Iterable):
                if verbose:
                    print(
                        "New parm: {} \t- new parm not iterable unlike old parm *****".format(
                            key
                        )
                    )
                tests.append(False)
...
def copy_dataset(h5_orig_dset, h5_dest_grp, alias=None, verbose=False):
    ...
    if not isinstance(h5_dest_grp, (h5py.File, h5py.Group)):
        raise TypeError(
            "'h5_dest_grp' should either be a h5py.File or " "h5py.Group object"
        )
    ...
    if alias in h5_dest_grp.keys():
        ...
        if not isinstance(h5_new_dset, h5py.Dataset):
            raise TypeError(
                ...
                " name which is not a dataset".format(h5_dest_grp, h5_new_dset)
            )
        ...
        if da_source.shape != da_dest.shape:
            raise ValueError(
                "Existing dataset: {} has a different shape "
                "compared to the original dataset: {}"
                "".format(h5_new_dset, h5_orig_dset)
            )
        if not da.allclose(da_source, da_dest):
            raise ValueError(
                "Existing dataset: {} has different contents"
                "compared to the original dataset: {}"
                "".format(h5_new_dset, h5_orig_dset)
            )
    else:
        ...
        if h5_orig_dset.file.driver == "mpio":
            if kwargs.pop("compression", None) is not None:
                warn(
                    "This HDF5 file has been opened wth the "
        ...
        if verbose:
            print(
                "Creating new HDF5 dataset named: {} at: {} with"
        ...
        da.to_hdf5(
            h5_new_dset.file.filename, {h5_new_dset.name: lazy_load_array(h5_orig_dset)}
        )
    if verbose:
        print(
            "Copying simple attributes of original dataset: {} to "
...
def copy_linked_objects(h5_source, h5_dest, verbose=False):
    ...
    if same_file:
        warn(
            "{} and {} are in the same HDF5 file. Consider copying references"
    ...
    for link_obj_name in h5_source.attrs.keys():
        ...
        if isinstance(h5_orig_obj, h5py.Reference) and not isinstance(
            h5_orig_obj, h5py.RegionReference
        ):
            h5_orig_obj = h5_source.file[h5_orig_obj]
            if verbose:
                print(
                    "Attempting to copy object linked to source: {} as {}"
            ...
            if link_obj_name in h5_dest_grp.keys():
                ...
                if type(h5_dest_grp[link_obj_name]) != type(h5_orig_obj):
                    ...
                    _ = copy_dataset(
                        h5_orig_obj, h5_dest_grp, alias=link_obj_name, verbose=verbose
                    )
                    h5_dest.attrs[link_obj_name] = h5_new_obj.ref
                    continue
                elif isinstance(h5_new_obj, h5py.Group):
                    raise ValueError(
                        "Destination already contains another "
                        "HDF5 group: {} with the same name as "
                        "the source: {}".format(h5_new_obj, h5_orig_obj)
                    )
                else:
                    raise NotImplementedError(
                        "Unable to copy {} objects yet"
...
def copy_region_refs(h5_source, h5_target):
    ...
    if not all(
        [isinstance(h5_source, h5py.Dataset), isinstance(h5_target, h5py.Dataset)]
    ):
    ...
    for key in h5_source.attrs.keys():
        ...
        if are_main:  # TODO: Region Reference copy not implemented
            ...
            ref_inds[:, 1, 0][ref_inds[:, 1, 0] > h5_spec_inds.shape[0]] = (
                h5_spec_inds.shape[0] - 1
            )
