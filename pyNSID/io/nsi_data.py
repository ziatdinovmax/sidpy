...
class NSIDataset(h5py.Dataset):
    ...
    """
        ...
        self.data_descriptor = "{} ({})".format(
            get_attr(self, "quantity"), get_attr(self, "units")
        )
        ...
        if self.dtype.fields is not None:
            usid_str = "\n".join(
                [
                    usid_str,
                    "Data Fields:",
                    "\t" + ", ".join([field for field in self.dtype.fields]),
                ]
            )
        else:
